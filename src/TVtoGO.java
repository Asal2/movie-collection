...
public class TVtoGO implements ActionListener, ListSelectionListener {
	...
	private static ArrayList<VideoItem> readFile(Scanner input) {
		...
		while (input.hasNextLine()) {
			...
			if (type.equals("M")) {
				Movie movie = new Movie(data[0], data[1], data[2], data[3], Integer.parseInt(data[4]),
						Integer.parseInt(data[5]), Double.parseDouble(data[6]), Integer.parseInt(data[7]));
//				System.out.println(movie);
				contentList.add(movie);
			} else if (type.equals("D")) {
				Documentary documentary = new Documentary(data[0], data[1], data[2], data[3],Integer.parseInt(data[4]),
						Integer.parseInt(data[5]), Double.parseDouble(data[6]), Integer.parseInt(data[7]));
//				System.out.println(documentary);
				contentList.add(documentary);
